...
SPI spi(A6, A5, A4);
MLX90363 angle_sensor_M2(D7, &spi);
MLX90363 angle_sensor_M1(A0, &spi);
MLX90363 angle_sensor_MT(D6, &spi);
...
void init_pids()
{
    speed_M1_pid.set_Kp(2);
    speed_M1_pid.set_Ki(8);
...
{
    sensed_drive_angle = angle_sensor_MT.Get_Angel() * TICKS2REV;
    if (!did_set_offset) {
...
{
    decoder_count_M1 = angle_sensor_M1.Get_Angel();
    decoder_count_M2 = angle_sensor_M2.Get_Angel();
    decoder_count_MT = TICKS_PER_REV_DDRIVE - angle_sensor_MT.Get_Angel();
...
int main()
{
    ...
    while (1) {
        ...
        if (loop_time_duration >= LOOPTIME) {
            ...
            angle_sensor_M1.Poll();
            angle_sensor_M2.Poll();
            angle_sensor_MT.Poll();
